...
const buildBentoJsonFromLinktree = (pageProps: LinktreePageProps) => {
  ...
  const json: BentoJSON = {
    ...
    name: profile.name.replace(/^@/, '').trim() || 'Linktree Import',
    version: '1.0',
...
export const importLinktreeToBento = async (input: string): Promise<LinktreeImportResult> => {
  if (!isImportEnabled()) {
    throw new Error(
      'Import feature is disabled. Set NEXT_PUBLIC_ENABLE_IMPORT=true to enable it.'
    );
  }

  const sourceUrl = normalizeLinktreeInput(input);
  const endpoint = `/api/import/linktree?url=${encodeURIComponent(sourceUrl)}`;
  ...
  return {
    bento: await importBentoFromJSON(json),
    importedCount,
    skippedCount,
    warnings,
  };
